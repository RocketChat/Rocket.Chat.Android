...
public class RocketChatWSAPI {
    ...
    public Task<DDPClientCallback.RPC> sendMessage(final String roomId, String msg, @Nullable JSONObject extraParams) throws JSONException {
        ...
        if (extraParams != null) {
            ...
            if (!extraParams.isNull("groupable"))
                param.put("groupable", extraParams.getBoolean("groupable"));
            if (!extraParams.isNull("attachments"))
                param.put("attachments", extraParams.getJSONArray("attachments"));
        }
    ...
    public Task<DDPClientCallback.RPC> getSubscriptions() throws JSONException {
        JSONArray params = new JSONArray()
                .put(new JSONObject().put("$date", 0));
        return mDDPClient.rpc("subscriptions/get", params, generateId("subscriptions-get"));
    }

    public Task<DDPClientCallback.RPC> getRooms() throws JSONException {
        JSONArray params = new JSONArray()
                .put(new JSONObject().put("$date", 0));
